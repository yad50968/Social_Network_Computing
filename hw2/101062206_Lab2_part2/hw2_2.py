...
with open("sentences", "r") as ins:
    for line in ins:
        tmp_line = line.split('-')
        list_one.append(tmp_line[0])
        tmp_list_two = tmp_line[1].split('|');
        list_two.append(tmp_list_two)
ins.close()
    ...
    def message(self, msg):
        ...
        if msg['type'] in ('chat', 'normal'):
            ...
            for i in range(0,len(list_one)):
                # 尋找是否有相對應的字串
                m1 =  re.search(list_one[i],message)
                
                if m1:
                    #如果有 隨機挑選一個回應
                    choose = random.randint(0, 2)
                    print(choose)
                    msg.reply(list_two[i][choose]).send()
...
if __name__ == '__main__':
    ...
    #xmpp = EchoBot('yad12345@adastra.re', '000000')
