...
class EchoBot(sleekxmpp.ClientXMPP):
    ...
    def start(self, event):
        ...
        self.send_presence()
        self.get_roster()
    def change(self,charchter):
        

        return int(charchter) 
    def message(self, msg):
        ...
        if msg['type'] in ('chat', 'normal'):
            ...
            # 如果輸入是help 則顯示使用教學
            if(message == 'help'):
                helpmessage = 'add :  add 1 2 4   then it will answer 1+2+4\n'+'minus: minus 10 5 3  then it will answer 10-5-3\n'+ 'multiply :  multiply 1 2 4   then it will answer 1*2*4\n'+ 'divide: divide 10 5 3  then it will answer 10/5/3\n'
                ...
                ans = int(message_list[1])
                if message_list[0] == 'add':

                    for i in range(2,len(message_list)):
                        tmp = int(message_list[i])
                        ans = ans + tmp

                      
                elif message_list[0] == 'minus':

                    for i in range(2,len(message_list)):
                        tmp = int(message_list[i])
                        ans = ans - tmp
                        
                elif message_list[0] == 'multiply':

                    for i in range(2,len(message_list)):
                        tmp = int(message_list[i])
                        ans = ans * tmp
                        
                elif message_list[0] == 'divide':
...
if __name__ == '__main__':
    ...
    #xmpp = EchoBot('yad12345@xmpp.jp', '000000')
